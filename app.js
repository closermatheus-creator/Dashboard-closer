...
// ==========================================================================
function renderComparativo(sdrStats, closerStats) {
    const section = document.getElementById("admin-comparativo-section");
    if (!section) return;

    // Mostra a seção (só admin chega aqui)
    section.style.display = "flex";

    // ── COMPARATIVO SDRs ──
    const sdrContainer = document.getElementById("comparativo-sdr-container");
    if (sdrContainer) {
        sdrContainer.innerHTML = "";
        const sdrEntries = Object.entries(sdrStats);

        if (sdrEntries.length === 0) {
            sdrContainer.innerHTML = `<p style="font-size:11px;color:var(--text-muted);grid-column:1/-1;">Nenhum dado de SDR no mês.</p>`;
        } else {
            // Ordena por agendadas desc para ranking visual
            sdrEntries.sort((a, b) => b[1].agendadas - a[1].agendadas);
            const maxAgendadas = Math.max(...sdrEntries.map(([,s]) => s.agendadas), 1);

            sdrEntries.forEach(([nome, ss], idx) => {
                const conv = ss.agendadas > 0 ? Math.round((ss.fechados / ss.agendadas) * 100) : 0;
                ...
                const card = document.createElement("div");
                card.style.cssText = "background:rgba(245,158,11,0.04);border:1px solid rgba(245,158,11,0.2);border-top:2px solid var(--warning-clean);border-radius:8px;padding:12px;";
                card.innerHTML = `
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                        <div style="display:flex;align-items:center;gap:6px;">
                            <div style="width:28px;height:28px;border-radius:50%;background:rgba(245,158,11,0.15);display:flex;align-items:center;justify-content:center;">
                                <i class="fa-solid fa-headset" style="color:var(--warning-clean);font-size:11px;"></i>
                            </div>
                            <div>
                                <div style="font-size:12px;font-weight:800;">${medal} ${nome}</div>
                                <div style="font-size:9px;color:var(--text-muted);">SDR · Mês Atual</div>
                            </div>
                        </div>
                        <span style="font-size:9px;font-weight:800;padding:2px 6px;border-radius:4px;background:rgba(245,158,11,0.1);color:var(--warning-clean);">#${idx+1}</span>
                    </div>
                    ...
                    <div style="margin-bottom:10px;">
                        ...
                        <div style="background:rgba(255,255,255,0.04);border-radius:4px;height:6px;overflow:hidden;">
                            <div style="width:${barWidth}%;height:100%;background:var(--warning-clean);border-radius:4px;transition:width 0.5s;"></div>
                        </div>
                    </div>
                ...
                `;
                sdrContainer.appendChild(card);
            });
        }
    }

    // ── COMPARATIVO CLOSERS ──
    const closerContainer = document.getElementById("comparativo-closer-container");
    if (closerContainer) {
        closerContainer.innerHTML = "";
        const closerEntries = Object.entries(closerStats);

        if (closerEntries.length === 0) {
            closerContainer.innerHTML = `<p style="font-size:11px;color:var(--text-muted);grid-column:1/-1;">Nenhum dado de closer no mês.</p>`;
        } else {
            // Ordena por faturamento desc
            closerEntries.sort((a, b) => b[1].fat - a[1].fat);
            const maxFat = Math.max(...closerEntries.map(([,c]) => c.fat), 1);
            const medals = ['🥇','🥈','🥉'];

            closerEntries.forEach(([email, cs], idx) => {
                const conv = cs.total > 0 ? Math.round((cs.fechados / cs.total) * 100) : 0;
                ...
                card.innerHTML = `
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                        <div style="display:flex;align-items:center;gap:6px;">
                            ...
                            <div>
                                <div style="font-size:12px;font-weight:800;">${medal} ${primeiroNome}</div>
                    ...
                    <!-- Conversão geral -->
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-top:1px solid var(--border-color);border-bottom:${(pctMeta !== null) ? '1px solid var(--border-color)' : 'none'};margin-bottom:${(pctMeta !== null) ? '8px' : '0'};">
                        <div>
                            <span style="font-size:8px;color:var(--text-muted);font-weight:700;">CONVERSÃO</span>
                            <span style="font-size:15px;font-weight:800;margin-left:4px;color:${corConv};">${conv}%</span>
                        </div>
                        <div style="font-size:8px;color:var(--text-muted);">${cs.pitches} pitches realizados</div>
                    </div>
                ...
                `;
                closerContainer.appendChild(card);
            });
        }
    }
}
