...
function initRealTimeListener() {
    ...
    onSnapshot(q, (snapshot) => {
        localLeadsCache = [];
        snapshot.forEach((doc) => { localLeadsCache.push({ id: doc.id, ...doc.data() }); });
        renderLeadsTable();
        window.populateSdrFilterOptions();
        window.calculateAdvancedMetrics();
...
function renderLeadsTable() {
    ...
    if (localLeadsCache.length === 0) {
        container.innerHTML = `<tr><td colspan="9" class="text-muted" style="text-align: center; padding: 20px;">Nenhum lead na mesa.</td></tr>`;
        return;
    }
    ...
    localLeadsCache.forEach((lead) => {
        ...
        tr.innerHTML = `
            <td onclick="window.editLead('${lead.id}')" style="cursor: pointer;" title="Editar lead">
            ...
            <td>${dataFormatada}</td>
            <td><div style="display:flex; flex-direction:column; gap:2px;"><span>Decisor: <span class="p-badge ${badgeDecisor}">${lead.decisor || 'Não'}</span></span><span class="p-badge ${badgeDiag}" style="text-align:center;">${lead.statusDiag || 'Aprovado'}</span></div></td>
            <td><div style="display:flex; flex-direction:column; gap:2px;"><span>Pitch: <strong>${lead.pitch}</strong></span><span class="lead-co-name">Reag. 2ª Call: ${lead.reagendado}</span></div></td>
            ...
            <td>
                <div class="btn-action-row">
                    <button class="btn-sm-note" onclick="window.openNotesModal('${lead.id}')" title="Notas"><i class="fa-solid fa-comment-dots"></i></button>
                    <button class="btn-action edit" onclick="window.editLead('${lead.id}')" title="Editar"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button class="btn-action delete" onclick="window.deleteLead('${lead.id}')" title="Deletar"><i class="fa-solid fa-trash-can"></i></button>
                </div>
            </td>
        `;
...
window.openNewLeadModal = function() {
    ...
    document.getElementById("form-modal-title").innerHTML = `<i class="fa-solid fa-user-plus"></i> Inserir Lead`;
...
window.handleLeadFormSubmit = async function(e) {
    ...
    const leadData = {
        nome: document.getElementById("form-nome").value, empresa: document.getElementById("form-empresa").value, faturamento: document.getElementById("form-faturamento").value, sdr: document.getElementById("form-sdr").value.trim() || "Direto", dataReuniao: document.getElementById("form-data-reuniao").value, origem: document.getElementById("form-origem").value, decisor: document.getElementById("form-decisor").value, statusDiag: document.getElementById("form-status-diag").value, pitch: document.getElementById("form-pitch").value, reagendado: document.getElementById("form-reagendado").value, desfecho: document.getElementById("form-desfecho").value, valor: parseFloat(document.getElementById("form-valor").value || 0), dataPagamento: document.getElementById("form-data-pagamento").value, motivoPerda: document.getElementById("form-motivo-perda").value, observacoes: id ? (localLeadsCache.find(l => l.id === id)?.observacoes || "") : ""
    };
    try {
...
window.editLead = function(id) {
    ...
    if (!lead) return;
    document.getElementById("form-lead-id").value = lead.id; document.getElementById("form-nome").value = lead.nome || ""; document.getElementById("form-empresa").value = lead.empresa || ""; document.getElementById("form-faturamento").value = lead.faturamento || ""; document.getElementById("form-sdr").value = lead.sdr || ""; document.getElementById("form-data-reuniao").value = lead.dataReuniao || ""; document.getElementById("form-origem").value = lead.origem || "CRM"; document.getElementById("form-decisor").value = lead.decisor || "Sim"; document.getElementById("form-status-diag").value = lead.statusDiag || "Aprovado para Pitch"; document.getElementById("form-pitch").value = lead.pitch || "Sim"; document.getElementById("form-reagendado").value = lead.reagendado || "Não"; document.getElementById("form-desfecho").value = lead.desfecho || "Em Aberto"; document.getElementById("form-valor").value = lead.valor || 0; document.getElementById("form-data-pagamento").value = lead.dataPagamento || ""; document.getElementById("form-motivo-perda").value = lead.motivoPerda || "";
    document.getElementById("form-modal-title").innerHTML = `<i class="fa-solid fa-sliders"></i> Atualizar Dados`;
    window.toggleLossReasonField(); document.getElementById("lead-form-modal").style.display = "flex";
};

window.deleteLead = async function(id) { if (confirm("Tem certeza que deseja remover este lead?")) await deleteDoc(doc(db, "leads", id)); };

let currentActiveNotesLeadId = null;
window.openNotesModal = function(id) {
    const lead = localLeadsCache.find(l => l.id === id);
    if (!lead) return;
    currentActiveNotesLeadId = id; document.getElementById("modal-lead-name").innerText = lead.nome; document.getElementById("lead-notes-area").value = lead.observacoes || ""; document.getElementById("notes-modal").style.display = "flex"; document.getElementById("lead-notes-area").focus();
};
window.closeNotesModal = function() { document.getElementById("notes-modal").style.display = "none"; currentActiveNotesLeadId = null; };
...
window.calculateAdvancedMetrics = function() {
    ...
    localLeadsCache.forEach(lead => {
        ...
        if (passouFiltroSdr) {
            funnelTotal++;
            if (lead.statusDiag === "No-Show") funnelNoShow++; else if (lead.statusDiag === "Aprovado para Pitch") funnelQualificados++; else if (lead.statusDiag === "Reprovado") funnelReprovados++;
            ...
            if (lead.origem === "I.A Yara") { yaraTotal++; if (lead.desfecho === "Fechado" || lead.desfecho === "Downsell") yaraFechados++; } else if (lead.origem === "CRM") { crmTotal++; if (lead.desfecho === "Fechado" || lead.desfecho === "Downsell") crmFechados++; }
...
function renderLossHistogram(objetoPerdas) {
    ...
    if (totalPerdas === 0) { container.innerHTML = `<p class="text-muted" style="padding:10px 0; font-size:11px;">Nenhum lead perdido.</p>`; return; }
    container.innerHTML = "";
    ...
    Object.keys(objetoPerdas).forEach(chave => {
        const quantidade = objetoPerdas[chave]; const percentualBarra = (quantidade / maxPerdas) * 100;
...
async function listUpcomingEvents() {
    try {
        ...
        if (!events || events.length === 0) { container.innerHTML = '<p class="text-muted" style="padding:15px; text-align:center;">Nenhum evento encontrado nos próximos 7 dias.</p>'; document.getElementById('calendar-modal').style.display = 'flex'; return; }
        ...
        events.forEach((event) => {
            ...
            const titulo = event.summary || 'Reunião Sem Título'; const ehAgenciaRei = titulo.toLowerCase().includes('rei') || titulo.toLowerCase().includes('diagnostico') || titulo.toLowerCase().includes('call'); const classeLinha = ehAgenciaRei ? 'calendar-import-row suggested' : 'calendar-import-row'; const jaImportado = salvosCalendarIds.includes(event.id);
            const div = document.createElement('div'); div.className = classeLinha;
            div.innerHTML = `<div style="max-width:70%; display:flex; flex-direction:column; gap:2px;"><strong style="font-size:11px; color:var(--text-primary);">${titulo}</strong><span style="font-size:10px; color:var(--text-muted);"><i class="fa-solid fa-clock"></i> ${dataTexto}</span></div><div>${jaImportado ? `<span class="badge-count" style="background-color:rgba(255,255,255,0.05); color:var(--text-muted);">Importado</span>` : `<button class="btn-sm-note" onclick="window.importEventToForm('${btoa(unescape(encodeURIComponent(JSON.stringify(event))))}')" style="color:var(--neon-accent); border-color:var(--neon-accent);">Puxar</button>`}</div>`;
...
window.importEventToForm = function(base64Event) {
    ...
    const form = document.getElementById('lead-form'); const antigoIdInput = document.getElementById('form-calendar-id-holder'); if(antigoIdInput) antigoIdInput.remove();
    const hiddenIdInput = document.createElement('input'); hiddenIdInput.type = 'hidden'; hiddenIdInput.id = 'form-calendar-id-holder'; hiddenIdInput.value = event.id; form.appendChild(hiddenIdInput);
    const originalSubmit = window.handleLeadFormSubmit;
    window.handleLeadFormSubmit = async function(e) {
        e.preventDefault(); const id = document.getElementById("form-lead-id").value;
        const leadData = { nome: document.getElementById("form-nome").value, empresa: document.getElementById("form-empresa").value, faturamento: document.getElementById("form-faturamento").value, sdr: document.getElementById("form-sdr").value.trim() || "Direto", dataReuniao: document.getElementById("form-data-reuniao").value, origem: document.getElementById("form-origem").value, decisor: document.getElementById("form-decisor").value, statusDiag: document.getElementById("form-status-diag").value, pitch: document.getElementById("form-pitch").value, reagendado: document.getElementById("form-reagendado").value, desfecho: document.getElementById("form-desfecho").value, valor: parseFloat(document.getElementById("form-valor").value || 0), dataPagamento: document.getElementById("form-data-pagamento").value, motivoPerda: document.getElementById("form-motivo-perda").value, calendarEventId: event.id, observacoes: id ? (localLeadsCache.find(l => l.id === id)?.observacoes || "") : "" };
